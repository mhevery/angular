...
import './ng_dev_mode';
import {
  LText, LElement, LView, LContainer, LProjection,
  LNode, ViewState, LNodeFlags,
  LNodeInjector, ContainerState, ProjectionState, QueryState, MinificationDataValue, MinificationData
} from './interfaces';
import { Renderer3, RElement, RText, RComment } from './renderer';
import { isDifferent, stringify } from './util';
import { DirectiveDef, ComponentDef, ComponentTemplate } from './public_interfaces';
import { appendChild, insertChild, insertView, removeView, processProjectedNode } from './node_manipulation';
import { QueryList, QueryState_ } from './query';
import { assertEqual, assertNotEqual, assertLessThan, assertNotNull, assertNumber } from './assert';
import { assertNodeType } from './node_assert';
export { refreshQuery } from './query'

export const enum LifeCycleGuard {
  ON_INIT = 1,
  ON_DESTROY = 2,
  ON_CHANGES = 4
}
 ...
 */
export function createNode(index: number | null, type: LNodeFlags.Element, native: RElement | RText | null, viewState?: ViewState | null, attrs?: string[] | null): LElement
export function createNode(index: null, type: LNodeFlags.View, native: null, viewState: ViewState, attrs?: null): LView
export function createNode(index: number, type: LNodeFlags.Container, native: RComment, containerState: ContainerState, attrs?: null): LContainer
export function createNode(index: number, type: LNodeFlags.Projection, native: null, projectionState: ProjectionState, attrs?: null): LProjection
export function createNode(
  ...
  attrs?: string[] | null,
): LElement & LText & LView & LContainer & LProjection {
  const parent = isParent ? previousOrParentNode : previousOrParentNode && previousOrParentNode.parent as LNode;
  let query = (isParent ? currentQuery : previousOrParentNode && previousOrParentNode.query) || parent && parent.query && parent.query.child();
  ...
  if ((type & LNodeFlags.ViewOrElement) === LNodeFlags.ViewOrElement && isState) {
    // Bit of a hack to bust through the readonly because there is a circular dep between ViewState and LNode.
...
export function elementCreate(
  index: number,
  nameOrComponentDef?: string | ComponentDef<any>,
  attrs?: string[]
): RElement {
  let node: LElement;
  let native: RElement;
  if (nameOrComponentDef == null) {
    ...
    const name = isHostElement ? (nameOrComponentDef as ComponentDef<any>).tag : nameOrComponentDef as string;
    if (name === null) {
      ...
      node = createNode(index, LNodeFlags.Element, native,
...
export function elementHost(elementOrSelector: RElement | string) {
  ...
  const rNode = typeof elementOrSelector === 'string' ? renderer.querySelector(elementOrSelector) : elementOrSelector;
  if (ngDevMode && !rNode) {
 ...
 */
export function listenerCreate(eventName: string, listener: EventListener, minificationData?: MinificationData,
...
function outputCreate(outputs: (number|string)[], listener: Function): void {
  for (let i = 0; i < outputs.length; i+= 2) {
    ...
    const subscription = directives[(outputs[i] as number) << 1][outputs[i | 1]].subscribe(listener);
...
export function elementProperty<T>(index: number, propName: string, value: T | NO_CHANGE,
                                   inputData?: MinificationData): void {
 ...
 */
export function elementStyle<T>(index: number, styleName: string, value: T | NO_CHANGE, suffix?: string): void {
  if (value !== NO_CHANGE) {
    const lElement = nodes[index] as LElement;
    if (value == null) {
...
export function directiveLifeCycle(lifeCycle: LifeCycleGuard.ON_DESTROY, self: any, method: Function): void;
export function directiveLifeCycle(lifeCycle: LifeCycleGuard): boolean;
export function directiveLifeCycle(lifeCycle: LifeCycleGuard, self?: any, method?: Function): boolean {
...
export function viewCreate(viewBlockId: number): boolean {
  ...
  const existingView: LView | false =
    !creationMode && containerState.nextIndex < children.length && children[containerState.nextIndex];
...
export function viewEnd(): void {
  ...
  const previousView = containerState.nextIndex <= containerState.children.length
    ? containerState.children[containerState.nextIndex - 1] as LView : null;
...
export const refreshComponent: <T>(
  directiveIndex: number,
  elementIndex: number,
  template: ComponentTemplate<T>
) => void = function <T>(
  this: undefined | { template: ComponentTemplate<T> },
  directiveIndex: number,
  elementIndex: number,
  template: ComponentTemplate<T>
) {
  ...
  while (componentChild !== null) {
    if ((componentChild.flags & LNodeFlags.TYPE_MASK) === LNodeFlags.Projection) {
      ...
      for (let i = 0; i < previouslyProjectedNodes.length; i++) {
        processProjectedNode(projectedNodes, previouslyProjectedNodes[i], currentParent, currentView);
      }
    } else {
      processProjectedNode(projectedNodes, componentChild as LElement | LText | LContainer, currentParent, currentView);
    }
...
export type NO_CHANGE = { brand: 'no change detected' };

/**
 * A special value which designates that a value has not changed.
 */
export const NO_CHANGE: NO_CHANGE = { brand: 'no change detected' };
 ...
 * @param suffix
 */
export function bind2(prefix: string, v0: any, i0: string, v1: any, suffix: string): string | NO_CHANGE {
  let different: boolean;
  if (different = creationMode) {
 ...
 * @param suffix
 */
export function bind3(prefix: string,
  v0: any, i0: string, v1: any, i1: string, v2: any, suffix: string): string | NO_CHANGE {
  let different: boolean;
  if (different = creationMode) {
    ...
    if (different = (isDifferent(parts.v0, v0) || isDifferent(parts.v1, v1) || isDifferent(parts.v2, v2))) {
      parts.v0 = v0;
      parts.v1 = v1;
      parts.v2 = v2;
    }
 ...
 * @param suffix
 */
export function bind4(prefix: string,
  v0: any, i0: string, v1: any, i1: string, v2: any, i2: string, v3: any, suffix: string): string | NO_CHANGE {
  let different: boolean;
  if (different = creationMode) {
    ...
    if (different = (isDifferent(parts.v0, v0) || isDifferent(parts.v1, v1) || isDifferent(parts.v2, v2) || isDifferent(parts.v3, v3))) {
      parts.v0 = v0;
      parts.v1 = v1;
      parts.v2 = v2;
      parts.v3 = v3;
 ...
 * @param suffix
 */
export function bind5(prefix: string,
  v0: any, i0: string, v1: any, i1: string, v2: any, i2: string, v3: any, i3: string, v4: any, suffix: string): string | NO_CHANGE {
  let different: boolean;
  if (different = creationMode) {
 ...
 * @param suffix
 */
export function bind6(prefix: string,
  v0: any, i0: string, v1: any, i1: string, v2: any, i2: string, v3: any, i3: string, v4: any,
  i4: string, v5: any, suffix: string): string | NO_CHANGE {
  let different: boolean;
  if (different = creationMode) {
    ...
    if (different = (isDifferent(parts.v0, v0) || isDifferent(parts.v1, v1) || isDifferent(parts.v2, v2) || isDifferent(parts.v3, v3)
      || isDifferent(parts.v4, v4) || isDifferent(parts.v5, v5))) {
      parts.v0 = v0;
      parts.v1 = v1;
      parts.v2 = v2;
      parts.v3 = v3;
 ...
 * @param suffix
 */
export function bind7(prefix: string,
  v0: any, i0: string, v1: any, i1: string, v2: any, i2: string, v3: any, i3: string, v4: any,
  i4: string, v5: any, i5: string, v6: any, suffix: string): string | NO_CHANGE {
  let different: boolean;
  if (different = creationMode) {
    bindings[bindingIndex++] = { v0: v0, v1: v1, v2: v2, v3: v3, v4: v4, v5: v5, v6: v6 };
  } else {
    const parts: { v0: any, v1: any, v2: any, v3: any, v4: any, v5: any, v6: any } = bindings[bindingIndex++];
    if (v0 === NO_CHANGE) v0 = parts.v0;
    if (v1 === NO_CHANGE) v1 = parts.v1;
    if (v2 === NO_CHANGE) v2 = parts.v2;
    if (v3 === NO_CHANGE) v3 = parts.v3;
    ...
    if (different = (isDifferent(parts.v0, v0) || isDifferent(parts.v1, v1) || isDifferent(parts.v2, v2) || isDifferent(parts.v3, v3)
      || isDifferent(parts.v4, v4) || isDifferent(parts.v5, v5) || isDifferent(parts.v6, v6))) {
      parts.v0 = v0;
      parts.v1 = v1;
      parts.v2 = v2;
      parts.v3 = v3;
 ...
 * @param suffix
 */
export function bind8(prefix: string,
  v0: any, i0: string, v1: any, i1: string, v2: any, i2: string, v3: any, i3: string, v4: any,
  i4: string, v5: any, i5: string, v6: any, i6: string, v7: any, suffix: string): string | NO_CHANGE {
  let different: boolean;
  if (different = creationMode) {
    bindings[bindingIndex++] = { v0: v0, v1: v1, v2: v2, v3: v3, v4: v4, v5: v5, v6: v6, v7: v7 };
  } else {
    const parts: { v0: any, v1: any, v2: any, v3: any, v4: any, v5: any, v6: any, v7: any } = bindings[bindingIndex++];
    if (v0 === NO_CHANGE) v0 = parts.v0;
    if (v1 === NO_CHANGE) v1 = parts.v1;
    if (v2 === NO_CHANGE) v2 = parts.v2;
    if (v3 === NO_CHANGE) v3 = parts.v3;
