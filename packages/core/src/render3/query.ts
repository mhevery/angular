...
import { QueryList as IQueryList } from '../linker';
import { Observable } from 'rxjs/Observable';
import { QueryState, LNode, LContainer, LView, LNodeFlags } from './interfaces';
...
export class QueryState_ implements QueryState {
  ...
  track<T>(queryList: IQueryList<T>, predicate: Type<T> | any[], descend?: boolean): void {
    // TODO(misko): This is not right. In case of inherited state, a calling track will incorrectly mutate parent.
    if (descend) {
  ...
  child(): QueryState | null {
    ...
    if (this.shallow === null) {
      // DeepQuery: We can reuse the current state if the child state would be same as current state.
      return this;
    } else {
  ...
  while (predicate) {
    ...
    if (type) {
      ...
      for (let i = flags >> LNodeFlags.INDX_SHIFT,
        ii = i + ((flags & LNodeFlags.SIZE_MASK) >> LNodeFlags.SIZE_SHIFT); i < ii; i++) {
...
function createPredicate<T>(previous: QueryPredicate<any> | null, queryList: QueryList<T>, predicate: Type<T> | any[]): QueryPredicate<T> {
