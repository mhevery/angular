...
export function assertEqual<T>(actual: T, expected: T, condition: string, serializer?: ((v:T) => string)) {
...
export function assertThrow<T>(actual: T, expected: T, condition: string, operator: string, serializer: ((v:T) => string) = stringify ) {
  throw new Error(`ASSERT: expected ${condition} ${operator} ${serializer(expected)} but was ${serializer(actual)}!` );
}
