...
import { Injector, Type, TemplateRef as ITemplateRef, ViewContainerRef as IViewContainerRef, ElementRef as IElementRef, 
  EmbeddedViewRef as IEmbeddedViewRef, ViewRef as IViewRef, ComponentFactory, NgModuleRef as INgModuleRef, ComponentRef as IComponentRef } from '@angular/core';
import { LNodeFlags, LContainer, LNodeInjector } from './interfaces';
import { ComponentTemplate } from './public_interfaces';
import { getOrCreateNodeInjector, NG_ELEMENT_ID, BLOOM_SIZE } from './instructions';
...
export function bloomFindPossibleInjector(injector: LNodeInjector, bloomBit: number): LNodeInjector | null {
  ...
  while (di) {
    ...
    let value: number = bloomBit < 64
      ...
      : (bloomBit < 96 ? di.bf2 : di.bf3)
    if ((value & mask) === mask) {
      return di;
    }
    // See if the parent injectors may have the value
    value = bloomBit < 64
      ? (bloomBit < 32 ? di.cbf0 : di.cbf1)
      : (bloomBit < 96 ? di.cbf2 : di.cbf3)
...
export function injectTemplateRef(): ITemplateRef<any> {
  ...
  return di.templateRef || (di.templateRef = new TemplateRef<any>(injectElementRef(), data.template));
}
...
class TemplateRef<T> implements ITemplateRef<T> {
  ...
  createEmbeddedView(context: T): IEmbeddedViewRef<T> {
...
class ViewContainerRef implements IViewContainerRef {
  ...
  get(index: number): IViewRef | null {
    throw notImplemented();
  }
  length: number;
  createEmbeddedView<C>(templateRef: ITemplateRef<C>, context?: C | undefined,
  ...
  insert(viewRef: IViewRef, index?: number | undefined): IViewRef {
    throw notImplemented();
  }
  move(viewRef: IViewRef, currentIndex: number): IViewRef {
    throw notImplemented();
  }
  indexOf(viewRef: IViewRef): number {
    throw notImplemented();
  }
  remove(index?: number | undefined): void {
    throw notImplemented();
  }
  detach(index?: number | undefined): IViewRef | null {
    throw notImplemented();
  }
