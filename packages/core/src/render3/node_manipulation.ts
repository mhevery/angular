...
import {
  LContainer, LNode, LNodeFlags, LView, LText, LElement, ViewState, ContainerState, ProjectionState,
  ViewOrContainerState, LProjection
} from './interfaces';
import { RNode, RComment, RElement, RText } from './renderer';
...
export function findBeforeNode(index: number, state: ContainerState, native: RComment): RElement | RText | RComment {
  const children = state.children;
  // Find the node to insert in front of
  return index + 1 < children.length ? (children[index + 1].data.nodes[0] as LText | LElement | LContainer).native : native;
}

export function addRemoveViewFromContainer(container: LContainer, rootNode: LView, insertMode: true, beforeNode: RNode | null): void;
export function addRemoveViewFromContainer(container: LContainer, rootNode: LView, insertMode: false): void;
export function addRemoveViewFromContainer(container: LContainer, rootNode: LView, insertMode: boolean, beforeNode?: RNode | null): void {
  ngDevMode && assertNodeType(container, LNodeFlags.Container);
  ngDevMode && assertNodeType(rootNode, LNodeFlags.View);
  const parent = findNativeParent(container);
  let node: LNode | null = rootNode.data.nodes[0];
  if (parent) {
    while (node) {
      const type = node.flags & LNodeFlags.TYPE_MASK;
      let nextNode: LNode | null = null;
      if (type === LNodeFlags.Element) {
        insertMode ? parent.insertBefore(node.native!, beforeNode as RNode | null, true)
        ...
        insertMode ? parent.appendChild(node.native!) : parent.removeChild(node.native!);
        nextNode = childContainerData.children.length ? childContainerData.children[0].data.nodes[0] : null;
      } else if (type === LNodeFlags.Projection) {
        nextNode = (node as LProjection).data[0];
      } else {
        nextNode = (node as LView).data.nodes[0];
      }
      if (nextNode === null) {
...
export function getParentState(state: ViewOrContainerState, rootView: ViewState): ViewOrContainerState | null {
  let node;
  if ((node = (state as ViewState)!.node) && (node.flags & LNodeFlags.TYPE_MASK) === LNodeFlags.View) {
...
export function appendChild(parent: LNode, child: RNode | null, currentView: ViewState): boolean {
  ...
  //    - Current View, and parent view same => content => don't add -> parent component will re-project if needed.
  ...
  if (child !== null && (parent.flags & LNodeFlags.TYPE_MASK) === LNodeFlags.Element &&
    (parent.view !== currentView /* Crossing View Boundaries, it is Component, but add Element of View */
...
export function insertChild(node: LNode, currentView: ViewState) {
  ...
  if ((parent.flags & LNodeFlags.TYPE_MASK) === LNodeFlags.Element &&
    (parent.view !== currentView /* Crossing View Boundaries, its Component, but add Element of View */
      || parent.data === null /* Regular Element. */)
  ) {
    // We only add element if not in View or not projected.
...
export function processProjectedNode(
  projectedNodes: ProjectionState,
  node: LElement | LText | LContainer,
  currentParent: LView | LElement,
