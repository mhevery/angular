...
import {
  defineComponent, E, e, rC, rc, C, c, V, v, D, l, L, T, p, b, defineDirective, mergeInputsAndOutputs
} from '../../src/render3/index';
import { renderToHtml, containerEl } from './render_util';
import {EventEmitter} from '@angular/core';
import {LifeCycleGuard} from "../../src/render3/instructions";
...
describe('outputs', () => {
  ...
  it('should call component output function when event is emitted', () => {
    const OD0 = mergeInputsAndOutputs([0, ButtonToggle.ngComponentDef]);
    ...
    function Template(ctx: any, cm: boolean) {
      if (cm) {
        E(0, ButtonToggle.ngComponentDef); {
          ...
          L('change', ctx.onChange.bind(ctx), OD0);
        } e();
      }
  ...
  it('should fire event listeners along with outputs if they match', () => {
    ...
    const OD0 = mergeInputsAndOutputs([0, MyButton.ngDirectiveDef]);

    function Template(ctx: any, cm: boolean) {
      if (cm) {
        E(0, 'button'); {
          D(0, MyButton.ngDirectiveDef.n(), MyButton.ngDirectiveDef);
          L('click', ctx.onClick.bind(ctx), OD0);
        } e();
      }
    }
  ...
  it('should work with two outputs of the same name', () => {
    ...
    const OD0 = mergeInputsAndOutputs([0, ButtonToggle.ngComponentDef, 1, OtherDir.ngDirectiveDef]);
