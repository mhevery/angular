...
import { E, e, T, t, b, C, c, V, v, rC, rc } from '../../src/render3/index';
...
describe('JS control flow', () => {
  ...
  it('should work with containers with views as parents', () => {
    ...
    expect(renderToHtml(Template, {condition1: true, condition2: true})).toEqual('<div>hello</div>world');
    expect(renderToHtml(Template, {condition1: false, condition2: false})).toEqual('<div>hello</div>');
  ...
  it('should work with loop block', () => {
    ...
    ctx.data.push('d');
    expect(renderToHtml(Template, ctx)).toEqual('<ul><li>a</li><li>b</li><li>c</li><li>d</li></ul>');
  ...
  it('should work with nested loop blocks where nested container is a root node', () => {
    ...
    const ctx = {
      cafes: [
        {name: '1', entrees: ['a', 'b', 'c']},
        {name: '2', entrees: ['d', 'e', 'f']},
        {name: '3', entrees: ['g','h', 'i']}
      ]
    };

    expect(renderToHtml(Template, ctx)).toEqual('<div>Before<h2>1</h2>abc-<h2>2</h2>def-<h2>3</h2>ghi-After</div>');
  ...
  it('should work with loop blocks nested three deep', () => {
    ...
    const ctx = {
      cafes: [
        {name: '1', entrees: [{name: 'a', foods: [1,2]}, {name: 'b', foods: [3,4]}, {name: 'c', foods: [5,6]}]},
        {name: '2', entrees: [{name: 'd', foods: [1,2]}, {name: 'e', foods: [3,4]}, {name: 'f', foods: [5,6]}]}
      ]
    };
  ...
  it('should work with if/else blocks', () => {
    const ctx: { message: string | null, condition: boolean }
...
describe('JS for loop', () => {
  it('should work with sibling for blocks', () => {
    const ctx: { data1: string[] | null, data2: number[] | null }
        = { data1: ['a', 'b', 'c'], data2: [1, 2] };
...
describe('function calls', () => {
  it('should work', () => {
    ...
    expect(renderToHtml(Template, ctx)).toEqual('<div>Before<span>foo</span><span>bar</span>After</div>');
