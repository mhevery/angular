...
import {
  defineComponent, defineDirective,
  E, e, T, t, a, p, b, k, D, i, C, c, V, v, rC, rc
...
describe('exports', () => {
  ...
  it('should support component instance fed into directive', () => {
    ...
    const MyDirDef = defineDirective({
      type: MyDir,
      factory: () => new MyDir
    });
  ...
  it('should work with directives with exportAs set', () => {
    ...
    class SomeDir { name = 'Drew'; }
    const SomeDirDef = defineDirective({
      type: SomeDir,
      factory: () => new SomeDir
    });
  ...
  describe('forward refs', () => {
    ...
    it('should work inside a view container', () => {
      ...
      expect(renderToHtml(Template, { condition: true })).toEqual('<div>one<input value="one"></div>');
